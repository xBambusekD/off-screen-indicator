...
namespace PixelPlay.OffScreenIndicator {
    public class OffScreenIndicatorCore {
        ...
        /// <param name="screenPosition">Position of the target mapped to screen cordinates</param>
        /// <returns></returns>
        public static bool IsTargetVisible(Vector3 screenPosition, ScreenBounds screenOffset) {
            bool isTargetVisible = screenPosition.z > 0 && screenPosition.x > 0 && screenPosition.x < Screen.width && screenPosition.y > 0 && screenPosition.y < Screen.height;
            // Check if target is not occluded by manually set screen bounds/offset.
            if (isTargetVisible) {
                bool targetInLeftOffset = screenPosition.x < screenOffset.LSideWidth && screenPosition.y < screenOffset.LSideHeight;
                bool targetInRightOffset = screenPosition.x > (Screen.width - screenOffset.RSideWidth) && screenPosition.y < screenOffset.RSideHeight;
                return !targetInLeftOffset && !targetInRightOffset;
            }
            return isTargetVisible;
        }
        ...
        /// <param name="screenCentre">The screen  centre</param>
        /// <param name="screenBounds">The screen bounds</param>
        public static void GetArrowIndicatorPositionAndAngle(ref Vector3 screenPosition, ref float angle, Vector3 screenCentre, Vector3 screenBounds, ScreenBounds screenOffset) {
            // Our screenPosition's origin is screen's bottom-left corner.
            // But we have to get the arrow's screenPosition and rotation with respect to screenCentre.
            screenPosition -= screenCentre;
            ...
            if (screenPosition.x > 0) {
                // Keep the x screen position to the maximum x bounds and
                // find the y screen position using y = mx.
                float yBound = screenBounds.x * slope;
                if (yBound > (screenOffset.RSideHeight - screenCentre.y)) {
                    screenPosition = new Vector3(screenBounds.x, yBound, 0);
                } else {
                    screenPosition = new Vector3(screenBounds.x - screenOffset.RSideWidth, (screenBounds.x - screenOffset.RSideWidth) * slope, 0);
                }
            } else {
                float yBound = -screenBounds.x * slope;
                if (yBound > (screenOffset.LSideHeight - screenCentre.y)) {
                    screenPosition = new Vector3(-screenBounds.x, yBound, 0);
                } else {
                    screenPosition = new Vector3(-screenBounds.x + screenOffset.LSideWidth, (-screenBounds.x + screenOffset.LSideWidth) * slope, 0);
                }
            }
